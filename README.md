...
Database credentials are read only from `DATABASE_URL` and are not stored in source code. Session and persistence helpers live under `app/db`, while pipeline persistence will be connected in a later milestone.
